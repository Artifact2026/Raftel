...
// New: Prepare a node list containing isTEE information
  nodes_t enc_nodes;
  enc_nodes.num_nodes = num;
  i = 0;
  for (std::set<PID>::iterator it = pids.begin(); it != pids.end(); ++it, i++) {
    NodeInfo* info = this->nodes.find(*it);
    if (info != NULL) {
      enc_nodes.nodes[i].pid = *it;
      enc_nodes.nodes[i].isTEE = info->getIsTEE();
      if (DEBUG) std::cout << KBLU << nfo() << "node " << *it << " isTEE: " << (info->getIsTEE() ? "true" : "false") << KNRM << std::endl;
    } else {
      enc_nodes.nodes[i].pid = *it;
      enc_nodes.nodes[i].isTEE = false; // Default value
      if (DEBUG) std::cout << KLRED << nfo() << "couldn't find node " << *it << " info, default isTEE: false" << KNRM << std::endl;
    }
  }
...
#elif defined(BASIC_DAMYSUS)
  this->pnet.reg_handler(salticidae::generic_bind(&Handler::handle_newview_damysus,    this, _1, _2));
  this->pnet.reg_handler(salticidae::generic_bind(&Handler::handle_ldrprepare_damysus, this, _1, _2));
...
// unsigned int Handler::getLeaderOf(View v) { return 0;}
...
bool Handler::isTEE(PID id) {
  NodeInfo* info = this->nodes.find(id);
  if (info != NULL) {
      return info->getIsTEE();
  }
  return false; // Or throw errors as needed
}
...
Peers Handler::remove_from_peers_pure(PID id) {
  Peers ret;
...
void Handler::sendMsgNewViewComb(MsgNewViewComb msg, Peers recipients) {
  if (DEBUG1) std::cout << KBLU << nfo() << "sending:" << msg.prettyPrint() << "->" << recipients2string(recipients) << KNRM << std::endl;
#if defined(BASIC_DAMYSUS)
  this->pnet.multicast_msg(MsgNewViewDamysus(msg.data,msg.sign), getPeerids(recipients));
#else
  this->pnet.multicast_msg(msg, getPeerids(recipients));
#endif
}
...
void Handler::sendMsgLdrPrepareComb(MsgLdrPrepareComb msg, Peers recipients) {
  if (DEBUG1) std::cout << KBLU << nfo() << "sending:" << msg.prettyPrint() << "->" << recipients2string(recipients) << KNRM << std::endl;
#if defined(BASIC_DAMYSUS)
  this->pnet.multicast_msg(MsgLdrPrepareDamysus(msg.acc,msg.block,msg.sign), getPeerids(recipients));
#else
  this->pnet.multicast_msg(msg, getPeerids(recipients));
#endif
...
void Handler::sendMsgPrepareComb(MsgPrepareComb msg, Peers recipients) {
  if (DEBUG1) std::cout << KBLU << nfo() << "sending:" << msg.prettyPrint() << "->" << recipients2string(recipients) << KNRM << std::endl;
#if defined(BASIC_DAMYSUS)
  this->pnet.multicast_msg(MsgPrepareDamysus(msg.data,msg.signs), getPeerids(recipients));
#else
  this->pnet.multicast_msg(msg, getPeerids(recipients));
#endif
...
void Handler::sendMsgPreCommitComb(MsgPreCommitComb msg, Peers recipients) {
  if (DEBUG1) std::cout << KBLU << nfo() << "sending:" << msg.prettyPrint() << "->" << recipients2string(recipients) << KNRM << std::endl;
#if defined(BASIC_DAMYSUS)
  this->pnet.multicast_msg(MsgPreCommitDamysus(msg.data,msg.signs), getPeerids(recipients));
#else
  this->pnet.multicast_msg(msg, getPeerids(recipients));
#endif
  if (DEBUGT) printNowTime("sending MsgPreCommitComb");
}
void Handler::sendMsgCommitComb(MsgCommitComb msg, Peers recipients) {
  if (DEBUG1) std::cout << KBLU << nfo() << "sending:" << msg.prettyPrint() << "->" << recipients2string(recipients) << KNRM << std::endl;
#if defined(BASIC_DAMYSUS)
  this->pnet.multicast_msg(MsgCommitDamysus(msg.data,msg.signs), getPeerids(recipients));
#else
  this->pnet.multicast_msg(msg, getPeerids(recipients));
#endif
  if (DEBUGT) printNowTime("sending MsgCommitComb");
}
...
#if defined(BASIC_DAMYSUS)
  accum_t aout = {};
  onejusts_t jin; setOneJusts(justs,&jin);
  sgx_status_t ret;
...
Just Handler::callTEEprepareComb(Block block, Accum acc, NewViewProofComb proof) {
  ...
  auto start = std::chrono::steady_clock::now();
#if defined(BASIC_DAMYSUS)
  just_t jout = {}; accum_t ain; hash_t hin;
  setAccum(acc,&ain); setHash(block.hash(),&hin);
  sgx_status_t ret;
...
#elif defined(BASIC_DAMYSUS)
  Just j = callTEEsignComb();
  if (j.isSet() && j.getSigns().getSize() == 1) {
    MsgNewViewComb msg(j.getRData(),j.getSigns().get(0));
    if (amCurrentLeader()) { handleNewviewComb(msg); }
    else { sendMsgNewViewComb(msg,recipients); }
  }
...
Accum Handler::newviews2accComb(const std::set<MsgNewViewComb>& newviews, unsigned int requiredSize) {
  ...
  unsigned int i = 0;
  for (auto it = newviews.begin(); it != newviews.end() && i < MAX_NUM_SIGNATURES; ++it, i++) {
      MsgNewViewComb msg = *it;
      if (i == 0) { rdata = msg.data; ss.add(msg.sign); }
      else { if (msg.data == rdata) { ss.add(msg.sign); } }
...
// For leader to send prepare message and receive response
void Handler::processQC(const std::set<MsgNewViewComb>& newviews, const std::string& qcType) {
  ...
  Accum acc;
  if (qcType == "TEE"){
    acc = newviews2accComb(newviews, this -> tqsize);
  }else if (qcType == "Normal"){
    acc = newviews2accComb(newviews, this -> qsize);
  }
  if (!acc.isSet()) {
      if (DEBUG2) std::cout << KBLU << nfo() << "bad accumulator for " << qcType << KNRM << std::endl;
      return;
  }
  Block block = createNewBlock(acc.getPreph());
  ...
  if (DEBUG1) std::cout << KBLU << nfo() << "ldr-prepare-" << qcType << ":" << msgPrep.signs.getSize() << KNRM << std::endl;
  ...
  if (msgPrep.signs.getSize() == 1) {
      ...
      // We store our own commit in the log
      if(isTEE(this->myid)){
        this->log.storeTEEPrepComb(msgPrep);
      } else{
        this->log.storePrepComb(msgPrep);
      }
      // if (this->log.storeTEEPrepComb(msgPrep) == this->tqsize || this->log.storePrepComb(msgPrep) == this->qsize) {
      //     decideComb(msgPrep.data, "TEE");
      // }
  }
}
...
void Handler::handleNewviewComb(MsgNewViewComb msg) {
  ...
  if (v >= this->view && amLeaderOf(v) && verifyNewViewComb(msg)) {
    ...
    if (isTEE(msg.sign.getSigner())){
      if (DEBUG1) std::cout << KRED << nfo() << "handle tee message:" << msg.prettyPrint() << KNRM << std::endl;
      // if ((this->log.storeTEENvComb(msg) == this->tqsize || this->log.storeNvComb(msg) == this->qsize) && v == this->view) { prepareComb();}
      teecount = this->log.storeTEENvComb(msg);
    }else{
      if (DEBUG1) std::cout << KRED << nfo() << "handle nontee message:" << msg.prettyPrint() << KNRM << std::endl;
...
// run by the leader to handle prepare message from backups
  ...
  if (v == this->view) {
    if (amLeaderOf(v)) {
      ...
      if (isTEE(msg.signs.get(0).getSigner())) {
        ...
        teecount = this->log.storeTEEPrepComb(msg);
        if (teecount != 0){ncount = (this->log).getPrepareComb(v, this->qsize).getSize();}
      } else {
        if (DEBUGH) std::cout << KRED << nfo() << "handle non-tee message:" << msg.prettyPrint() << KNRM << std::endl;
        ncount = this->log.storePrepComb(msg);
      ...
      if (this->fastQC && (teecount >= this->tqsize) && this->phase == Phase::PREPARE) {
        ...
        if(isTEE(getCurrentLeader())){
          ...
          this->phase = Phase::PRECOMMIT;
          preCommitComb(data, "TEE");
        }
        
      } else if (((teecount + ncount) == this->qsize) && (this->phase == Phase::PREPARE)){
        if (DEBUGH) std::cout << KRED << nfo() << "enough non-tee  message:" << msg.prettyPrint() << KNRM << std::endl;
        if(isTEE(getCurrentLeader())){
          this->phase = Phase::NEWVIEW;
          decideComb(data, "Normal");
        }else{
          this->phase = Phase::PRECOMMIT;
          preCommitComb(data, "Normal");
        }
      }
...
// // For backups to respond to MsgPrepareComb messages received from leaders
void Handler::respondToPrepareComb(MsgPrepareComb msg) {
  PID leader = getCurrentLeader();
  if (verifyPrepareCombCert(msg)) {
    if (DEBUG1) { std::cout << KMAG << nfo() << "response to prepare" << KNRM << std::endl; }
    if(isTEE(leader)){
      ...
      Peers recipients = keep_from_peers(getCurrentLeader());
      if (DEBUG1) { std::cout << KMAG << nfo() << "leader:" <<getCurrentLeader() << KNRM << std::endl; }
...
// For leaders to send pre-commit certificates to backups at the beginning the decide phase
void Handler::decideComb(RData data, const std::string& qcType) {
  ...
  if(isTEE(getCurrentLeader())){
    ...
    if (verifyCombQC(data, signs, PH1_PREPARE)) {
      ...
      Peers recipients = remove_from_peers_pure(this->myid);
      sendMsgPrepareComb(finalMsg, recipients); // Send a prepared message with a qsize signature as a "commit".
    } 
  }else{
    if (qcType == "TEE"){
...
void Handler::preCommitComb(RData data, const std::string& qcType) {
  ...
  if (verifyCombQC(data, signs, PH1_PREPARE)) {
    ...
    // We store our own commit in the log
    if(isTEE(this->myid)){
      this->log.storeTEEPcComb(msgPc);
    } else{
      this->log.storePcComb(msgPc);
    }
    // if (this->qsize <= this->log.storePcComb(msgPc) || this->tqsize <= this->log.storeTEEPcComb(msgPc)) {
    //   commitComb(justPc.getRData(), "TEE");
    // }
  }
}

// For leaders to send commit certificates to backups at the beginning of the execute phase
void Handler::commitComb(RData data, const std::string& qcType) {
  View view = data.getPropv();
  Signs signs;
  if (qcType == "TEE"){
    signs = (this->log).getTEEPrecommitComb(view,this->tqsize); //  Obtain precommit signature from log
  }else if (qcType == "Normal"){
...
void Handler::respondToPreCommitComb(MsgPreCommitComb msg) {
  ...
  Peers recipients = keep_from_peers(getCurrentLeader());
  sendMsgCommitComb(msgc, recipients);
}


void Handler::handlePreCommitComb(MsgPreCommitComb msg) {
  auto start = std::chrono::steady_clock::now();
  if (DEBUG1) std::cout << KBLU << nfo() << "handling: pre-commit" << KNRM << std::endl;
  if (DEBUG1) std::cout << KBLU << nfo() << "handling:" << msg.prettyPrint() << KNRM << std::endl;
  RData data = msg.data;
  View v = data.getPropv();
  if (v == this->view) {
    if (amLeaderOf(v)) {
      ...
      if (isTEE(msg.signs.get(0).getSigner())){
        ...
        if (DEBUGH) std::cout << KRED << nfo() << "handle non-tee message:" << msg.prettyPrint() << KNRM << std::endl;
        ncount = this->log.storePcComb(msg);
        if (ncount != 0){teecount = (this->log).getTEEPrecommitComb(v, this->tqsize).getSize();}
      }
      
      if ((teecount == this->tqsize)&&(this->phase == Phase::PRECOMMIT)){
        if (DEBUGH) std::cout << KRED << nfo() << "enough tee message:" << msg.prettyPrint() << KNRM << std::endl;
        this->phase = Phase::COMMIT;
        commitComb(data, "TEE");
      } else if (((teecount + ncount) == this->qsize)&&(this->phase == Phase::PRECOMMIT)){
        if (DEBUGH) std::cout << KRED << nfo() << "enough non-tee  message:" << msg.prettyPrint() << KNRM << std::endl;
        this->phase = Phase::COMMIT;
        commitComb(data, "Normal");
      }
    } else {
...
void Handler::handleCommitComb(MsgCommitComb msg) {
  auto start = std::chrono::steady_clock::now();
  if (DEBUG1) std::cout << KBLU << nfo() << "handling:" << msg.prettyPrint() << KNRM << std::endl;
  RData data = msg.data;
  View v = data.getPropv();
  if (v == this->view) {
    if (amLeaderOf(v)) {
      ...
      if ((teecount == this->tqsize) && (this->phase == Phase::COMMIT)){
        ...
        this->phase = Phase::NEWVIEW;
        decideComb(data, "TEE");
      } else if (((teecount + ncount) == this->qsize) && (this->phase == Phase::COMMIT)){
        if (DEBUGH) std::cout << KRED << nfo() << "enough non-tee message:" << msg.prettyPrint() << KNRM << std::endl;
        this->phase = Phase::NEWVIEW;
        decideComb(data, "Normal");
      }
...
void Handler::handle_commitcomb(MsgCommitComb msg, const PeerNet::conn_t &conn) {
  handleCommitComb(msg);
}

// BASIC_DAMYSUS wire adapters.  The internal Cheap&Quick state machine uses
// the established Comb value objects, while the network opcodes and leader
// proposal layout remain isolated from the hybrid protocols.
void Handler::handle_newview_damysus(MsgNewViewDamysus msg, const PeerNet::conn_t &) {
  handleNewviewComb(MsgNewViewComb(msg.data,msg.sign));
}

void Handler::handle_ldrprepare_damysus(MsgLdrPrepareDamysus msg, const PeerNet::conn_t &) {
  handleLdrPrepareComb(MsgLdrPrepareComb(msg.acc,msg.block,NewViewProofComb(),msg.sign));
}

void Handler::handle_prepare_damysus(MsgPrepareDamysus msg, const PeerNet::conn_t &) {
  handlePrepareComb(MsgPrepareComb(msg.data,msg.signs));
}

void Handler::handle_precommit_damysus(MsgPreCommitDamysus msg, const PeerNet::conn_t &) {
  handlePreCommitComb(MsgPreCommitComb(msg.data,msg.signs));
}
