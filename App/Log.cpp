...
bool msgCommitCombFrom(std::set<MsgCommitComb> msgs, std::set<PID> signers) {
  for (std::set<MsgCommitComb>::iterator it=msgs.begin(); it!=msgs.end(); ++it) {
    MsgCommitComb msg = (MsgCommitComb)*it;
    std::set<PID> k = msg.signs.getSigners();
    for (std::set<PID>::iterator it2=k.begin(); it2!=k.end(); ++it2) {
      signers.erase((PID)*it2);
      if (signers.empty()) { return true; }
...
unsigned int Log::storeTEENvComb(MsgNewViewComb msg) {
  RData data = msg.data;
  View v = data.getPropv();
  PID signer = msg.sign.getSigner();

  std::map<View,std::set<MsgNewViewComb>>::iterator it1 = this->TEEnewviewsComb.find(v);
  if (it1 != this->TEEnewviewsComb.end()) { // There is already a record of this view
    std::set<MsgNewViewComb> msgs = it1->second;
  // Only add signers that have not appeared before
    bool exists = false;
    for (auto &m : msgs) {
      if (m.sign.getSigner() == signer) { exists = true; break; }
    }
    if (!exists) {
      msgs.insert(msg);
      this->TEEnewviewsComb[v] = msgs;
      return msgs.size();
    }
  } else {// There is no record of this view
    this->TEEnewviewsComb[v] = {msg};
    return 1;
  }
  return 0;
}

std::set<MsgNewViewComb> Log::getTEENewViewComb(View view, unsigned int n) {
  std::set<MsgNewViewComb> ret;
  std::map<View,std::set<MsgNewViewComb>>::iterator it1 = this->TEEnewviewsComb.find(view);
  if (it1 != this->TEEnewviewsComb.end()) {
    std::set<MsgNewViewComb> msgs = it1->second;
    for (std::set<MsgNewViewComb>::iterator it=msgs.begin(); ret.size() < n && it!=msgs.end(); ++it) {
      MsgNewViewComb msg = (MsgNewViewComb)*it;
      ret.insert(msg);
...
unsigned int Log::storeTEEPrepComb(MsgPrepareComb msg) {
  RData data = msg.data;
  View v = data.getPropv();
  std::set<PID> signers = msg.signs.getSigners();

  std::map<View,std::set<MsgPrepareComb>>::iterator it1 = this->TEEpreparesComb.find(v);
  if (it1 != this->TEEpreparesComb.end()) { // there is already an entry for this view
    std::set<MsgPrepareComb> msgs = it1->second;

    // We only add 'msg' to the log if the sender hasn't already sent a new-view message for this view
    if (!msgPrepareCombFrom(msgs,signers)) {
      msgs.insert(msg);
      this->TEEpreparesComb[v]=msgs;
      //if (DEBUG) { std::cout << KGRN << "updated entry; #=" << msgs.size() << KNRM << std::endl; }
      return msgs.size();
    }
  } else { // there is no entry for this view
    this->TEEpreparesComb[v]={msg};
    if (DEBUG) { std::cout << KGRN << "no entry for this view (" << v << ") before; #=1" << KNRM << std::endl; }
    return 1;
  }
...
unsigned int Log::storePcComb(MsgPreCommitComb msg) {
  ...
  if (it1 != this->precommitsComb.end()) { // there is already an entry for this view
    ...
    // We only add 'msg' to the log if the sender hasn't already sent a pre-commit message for this view
...
unsigned int Log::storeTEEPcComb(MsgPreCommitComb msg) {
  RData data = msg.data;
  View v = data.getPropv();
  std::set<PID> signers = msg.signs.getSigners();

  std::map<View,std::set<MsgPreCommitComb>>::iterator it1 = this->TEEprecommitsComb.find(v);
  if (it1 != this->TEEprecommitsComb.end()) { // there is already an entry for this view
    std::set<MsgPreCommitComb> msgs = it1->second;

    // We only add 'msg' to the log if the sender hasn't already sent a pre-commit message for this view
    if (!msgPreCommitCombFrom(msgs,signers)) {
      msgs.insert(msg);
      this->TEEprecommitsComb[v]=msgs;
      return msgs.size();
    }
  } else { // there is no entry for this view
    this->TEEprecommitsComb[v]={msg};
    if (DEBUG) { std::cout << KGRN << "no entry for this view (" << v << ") before; #=1" << KNRM << std::endl; }
    return 1;
  }
...
unsigned int Log::storeComComb(MsgCommitComb msg) {
  RData data = msg.data;
  View v = data.getPropv();
  std::set<PID> signers = msg.signs.getSigners();

  std::map<View,std::set<MsgCommitComb>>::iterator it1 = this->commitsComb.find(v);
  if (it1 != this->commitsComb.end()) { // there is already an entry for this view
    std::set<MsgCommitComb> msgs = it1->second;

    // We only add 'msg' to the log if the sender hasn't already sent a commit message for this view
    if (!msgCommitCombFrom(msgs,signers)) {
      msgs.insert(msg);
      this->commitsComb[v]=msgs;
      return msgs.size();
    }
  } else { // there is no entry for this view
    this->commitsComb[v]={msg};
    if (DEBUG) { std::cout << KGRN << "no entry for this view (" << v << ") before; #=1" << KNRM << std::endl; }
    return 1;
  }
...
unsigned int Log::storeTEEComComb(MsgCommitComb msg) {
  RData data = msg.data;
  View v = data.getPropv();
  std::set<PID> signers = msg.signs.getSigners();

  std::map<View,std::set<MsgCommitComb>>::iterator it1 = this->TEEcommitsComb.find(v);
  if (it1 != this->TEEcommitsComb.end()) { // there is already an entry for this view
    std::set<MsgCommitComb> msgs = it1->second;

    // We only add 'msg' to the log if the sender hasn't already sent a commit message for this view
    if (!msgCommitCombFrom(msgs,signers)) {
      msgs.insert(msg);
      this->TEEcommitsComb[v]=msgs;
      return msgs.size();
    }
  } else { // there is no entry for this view
    this->TEEcommitsComb[v]={msg};
    if (DEBUG) { std::cout << KGRN << "no entry for this view (" << v << ") before; #=1" << KNRM << std::endl; }
    return 1;
  }
...
// New: merge TEE and non-TEE prepare signatures with de-duplication up to n
Signs Log::getPrepareCombMerged(View view, unsigned int n) {
  Signs out;
  // Prefer TEE signs first, then fill with non-TEE
  Signs tee = getTEEPrepareComb(view, n);
  out.addUpto(tee, n);
  if (out.getSize() < n) {
    Signs non = getPrepareComb(view, n);
    out.addUpto(non, n);
  }
  return out;
}
...
Signs Log::getTEEPrepareComb(View view, unsigned int n) {
  Signs signs;
  std::map<View,std::set<MsgPrepareComb>>::iterator it1 = this->TEEpreparesComb.find(view);
  if (it1 != this->TEEpreparesComb.end()) { // there is already an entry for this view
    std::set<MsgPrepareComb> msgs = it1->second;
    for (std::set<MsgPrepareComb>::iterator it=msgs.begin(); signs.getSize() < n && it!=msgs.end(); ++it) {
      MsgPrepareComb msg = (MsgPrepareComb)*it;
...
Signs Log::getTEEPrecommitComb(View view, unsigned int n) {
  Signs signs;
  std::map<View,std::set<MsgPreCommitComb>>::iterator it1 = this->TEEprecommitsComb.find(view);
  if (it1 != this->TEEprecommitsComb.end()) { // there is already an entry for this view
    std::set<MsgPreCommitComb> msgs = it1->second;
    for (std::set<MsgPreCommitComb>::iterator it=msgs.begin(); signs.getSize() < n && it!=msgs.end(); ++it) {
      MsgPreCommitComb msg = (MsgPreCommitComb)*it;
...
Signs Log::getCommitComb(View view, unsigned int n) {
  Signs signs;
  std::map<View,std::set<MsgCommitComb>>::iterator it1 = this->commitsComb.find(view);
  if (it1 != this->commitsComb.end()) { // there is already an entry for this view
    std::set<MsgCommitComb> msgs = it1->second;
    for (std::set<MsgCommitComb>::iterator it=msgs.begin(); signs.getSize() < n && it!=msgs.end(); ++it) {
      MsgCommitComb msg = (MsgCommitComb)*it;
      Signs others = msg.signs;
      signs.addUpto(others,n);
    }
...
Signs Log::getTEECommitComb(View view, unsigned int n) {
  Signs signs;
  std::map<View,std::set<MsgCommitComb>>::iterator it1 = this->TEEcommitsComb.find(view);
  if (it1 != this->TEEcommitsComb.end()) { // there is already an entry for this view
    std::set<MsgCommitComb> msgs = it1->second;
    for (std::set<MsgCommitComb>::iterator it=msgs.begin(); signs.getSize() < n && it!=msgs.end(); ++it) {
      MsgCommitComb msg = (MsgCommitComb)*it;
...
// New: merge TEE and non-TEE precommit signatures with de-duplication up to n
Signs Log::getPrecommitCombMerged(View view, unsigned int n) {
  Signs out;
  // Prefer TEE signs first, then fill with non-TEE
  Signs tee = getTEEPrecommitComb(view, n);
  out.addUpto(tee, n);
  if (out.getSize() < n) {
    Signs non = getPrecommitComb(view, n);
    out.addUpto(non, n);
  }
  return out;
}

// New: merge TEE and non-TEE commit signatures with de-duplication up to n
Signs Log::getCommitCombMerged(View view, unsigned int n) {
  Signs out;
  // Prefer TEE signs first, then fill with non-TEE
  Signs tee = getTEECommitComb(view, n);
  out.addUpto(tee, n);
  if (out.getSize() < n) {
    Signs non = getCommitComb(view, n);
    out.addUpto(non, n);
  }
  return out;
}
