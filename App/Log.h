...
// New: Store the newview message from the TEE node under each view
  std::map<View, std::set<MsgNewViewComb>> TEEnewviewsComb;
  std::map<View,std::set<MsgPrepareComb>> preparesComb;
//New:
  std::map<View,std::set<MsgPrepareComb>> TEEpreparesComb;
  std::map<View,std::set<MsgPreCommitComb>> precommitsComb;
  std::map<View,std::set<MsgPreCommitComb>> TEEprecommitsComb;
  std::map<View,std::set<MsgCommitComb>> commitsComb;
  std::map<View,std::set<MsgCommitComb>> TEEcommitsComb;
...
// New: Store the newview message from the TEE node
  unsigned int storeTEENvComb(MsgNewViewComb msg);
  unsigned int storePrepComb(MsgPrepareComb msg);
//New:
  unsigned int storeTEEPrepComb(MsgPrepareComb msg);
  unsigned int storePcComb(MsgPreCommitComb msg);
  unsigned int storeTEEPcComb(MsgPreCommitComb msg);
  unsigned int storeComComb(MsgCommitComb msg);
  unsigned int storeTEEComComb(MsgCommitComb msg);
  ...
  // New: Obtain newview messages from TEE nodes for a certain view
  std::set<MsgNewViewComb> getTEENewViewComb(View view, unsigned int n);
  Signs getPrepareComb(View view, unsigned int n);
  // New: collect prepare signatures from TEE-only storage
  Signs getTEEPrepareComb(View view, unsigned int n);
  // New: merge TEE and non-TEE prepare signatures with de-duplication up to n
  Signs getPrepareCombMerged(View view, unsigned int n);
  Signs getPrecommitComb(View view, unsigned int n);
  Signs getTEEPrecommitComb(View view, unsigned int n);
  // New: merge TEE and non-TEE precommit signatures with de-duplication up to n
  Signs getPrecommitCombMerged(View view, unsigned int n);
  Signs getCommitComb(View view, unsigned int n);
  Signs getTEECommitComb(View view, unsigned int n);
  // New: merge TEE and non-TEE commit signatures with de-duplication up to n
  Signs getCommitCombMerged(View view, unsigned int n);
