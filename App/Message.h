...
struct MsgCommitComb {
  static const uint8_t opcode = HDR_COMMIT_COMB;
  salticidae::DataStream serialized;
  RData data;
  Signs signs;
  MsgCommitComb(const RData &data, const Signs &signs) : data(data),signs(signs) { serialized << data << signs; }
  MsgCommitComb(salticidae::DataStream &&s) { s >> data >> signs; }
  bool operator<(const MsgCommitComb& s) const {
    if (signs < s.signs) { return true; }
    return false;
  }
  std::string prettyPrint() {
    return "COMMIT-COMB[" + data.prettyPrint() + "," + signs.prettyPrint() + "]";
  }
  unsigned int sizeMsg() { return (sizeof(RData) + sizeof(Signs)); }
  void serialize(salticidae::DataStream &s) const { s << data << signs; }
};
