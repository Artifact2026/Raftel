...
void Nodes::addNode(PID id, HOST host, PORT rport, PORT cport, bool isTEE) {
  if (DEBUG) { std::cout << KMAG << "adding " << id << " to list of nodes" << KNRM << std::endl; }
  NodeInfo hp(id, host, rport, cport, isTEE);
  this->nodes[id] = hp;
}
...
Nodes::Nodes(std::string filename, unsigned int numNodes) {
  ...
  while (inFile && added < numNodes) {
    ...
    if (token) {
      ...
      token=strtok(NULL,delim);
      bool isTEE = false;
      if (token && strncmp(token, "isTEE:", 6) == 0) {
          isTEE = atoi(token+6) != 0;
      }

      if (DEBUG) std::cout << KMAG << "id: " << id << "; host: " << host << "; port: " << rport << "; port: " << cport << KNRM << std::endl;

      addNode(id,host,rport,cport, isTEE);
      added++;
    }
  }
