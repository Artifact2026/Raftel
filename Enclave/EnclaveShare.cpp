...
std::map<PID, bool> node_is_TEE;//node type
double cryptoTime;
...
sgx_status_t initialize_variables(PID *me, pids_t *others, unsigned int *q, unsigned int *tq, nodes_t *nodes) {
  ...
  // initialize node_is_TEE
  node_is_TEE.clear();
  for (unsigned int i = 0; i < nodes->num_nodes; i++) {
      PID pid = nodes->nodes[i].pid;
      bool isTEE = nodes->nodes[i].isTEE;
      node_is_TEE[pid] = isTEE;
  }
