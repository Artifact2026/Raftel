...
typedef struct ms_DAMYSUS_TEEsign_t {
	sgx_status_t ms_retval;
	just_t* ms_just;
} ms_DAMYSUS_TEEsign_t;

typedef struct ms_DAMYSUS_TEEprepare_t {
	sgx_status_t ms_retval;
	hash_t* ms_hash;
	accum_t* ms_acc;
	just_t* ms_res;
} ms_DAMYSUS_TEEprepare_t;

typedef struct ms_DAMYSUS_TEEstore_t {
	sgx_status_t ms_retval;
	just_t* ms_just;
	just_t* ms_res;
} ms_DAMYSUS_TEEstore_t;

typedef struct ms_DAMYSUS_TEEaccum_t {
	sgx_status_t ms_retval;
	onejusts_t* ms_js;
	accum_t* ms_res;
} ms_DAMYSUS_TEEaccum_t;

typedef struct ms_DAMYSUS_TEEaccumSp_t {
	sgx_status_t ms_retval;
	just_t* ms_just;
	accum_t* ms_res;
} ms_DAMYSUS_TEEaccumSp_t;
...
static sgx_status_t SGX_CDECL sgx_DAMYSUS_TEEsign(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_DAMYSUS_TEEsign_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_DAMYSUS_TEEsign_t* ms = SGX_CAST(ms_DAMYSUS_TEEsign_t*, pms);
	ms_DAMYSUS_TEEsign_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_DAMYSUS_TEEsign_t), ms, sizeof(ms_DAMYSUS_TEEsign_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	...
	_in_retval = DAMYSUS_TEEsign(_in_just);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_DAMYSUS_TEEprepare(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_DAMYSUS_TEEprepare_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_DAMYSUS_TEEprepare_t* ms = SGX_CAST(ms_DAMYSUS_TEEprepare_t*, pms);
	ms_DAMYSUS_TEEprepare_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_DAMYSUS_TEEprepare_t), ms, sizeof(ms_DAMYSUS_TEEprepare_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	hash_t* _tmp_hash = __in_ms.ms_hash;
	...
	_in_retval = DAMYSUS_TEEprepare(_in_hash, _in_acc, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_DAMYSUS_TEEstore(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_DAMYSUS_TEEstore_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_DAMYSUS_TEEstore_t* ms = SGX_CAST(ms_DAMYSUS_TEEstore_t*, pms);
	ms_DAMYSUS_TEEstore_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_DAMYSUS_TEEstore_t), ms, sizeof(ms_DAMYSUS_TEEstore_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	...
	_in_retval = DAMYSUS_TEEstore(_in_just, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_DAMYSUS_TEEaccum(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_DAMYSUS_TEEaccum_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_DAMYSUS_TEEaccum_t* ms = SGX_CAST(ms_DAMYSUS_TEEaccum_t*, pms);
	ms_DAMYSUS_TEEaccum_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_DAMYSUS_TEEaccum_t), ms, sizeof(ms_DAMYSUS_TEEaccum_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	onejusts_t* _tmp_js = __in_ms.ms_js;
	...
	_in_retval = DAMYSUS_TEEaccum(_in_js, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_DAMYSUS_TEEaccumSp(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_DAMYSUS_TEEaccumSp_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_DAMYSUS_TEEaccumSp_t* ms = SGX_CAST(ms_DAMYSUS_TEEaccumSp_t*, pms);
	ms_DAMYSUS_TEEaccumSp_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_DAMYSUS_TEEaccumSp_t), ms, sizeof(ms_DAMYSUS_TEEaccumSp_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	...
	_in_retval = DAMYSUS_TEEaccumSp(_in_just, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ACHILLES_CH_TEEsign(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ACHILLES_CH_TEEsign_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ACHILLES_CH_TEEsign_t* ms = SGX_CAST(ms_ACHILLES_CH_TEEsign_t*, pms);
	ms_ACHILLES_CH_TEEsign_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ACHILLES_CH_TEEsign_t), ms, sizeof(ms_ACHILLES_CH_TEEsign_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	...
	_in_retval = ACHILLES_CH_TEEsign(_in_just);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ACHILLES_CH_TEEverify(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ACHILLES_CH_TEEverify_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ACHILLES_CH_TEEverify_t* ms = SGX_CAST(ms_ACHILLES_CH_TEEverify_t*, pms);
	ms_ACHILLES_CH_TEEverify_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ACHILLES_CH_TEEverify_t), ms, sizeof(ms_ACHILLES_CH_TEEverify_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	size_t _len_just = sizeof(just_t);
	just_t* _in_just = NULL;
	unsigned int* _tmp_res = __in_ms.ms_res;
	size_t _len_res = sizeof(unsigned int);
	unsigned int* _in_res = NULL;
	sgx_status_t _in_retval;
	...
	if (_tmp_res != NULL && _len_res != 0) {
		...
		if ((_in_res = (unsigned int*)malloc(_len_res)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	_in_retval = ACHILLES_CH_TEEverify(_in_just, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ACHILLES_CH_TEEprepare(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ACHILLES_CH_TEEprepare_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ACHILLES_CH_TEEprepare_t* ms = SGX_CAST(ms_ACHILLES_CH_TEEprepare_t*, pms);
	ms_ACHILLES_CH_TEEprepare_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ACHILLES_CH_TEEprepare_t), ms, sizeof(ms_ACHILLES_CH_TEEprepare_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	cblock_t* _tmp_block = __in_ms.ms_block;
	...
	_in_retval = ACHILLES_CH_TEEprepare(_in_block, _in_hash, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ACHILLES_CH_TEEaccum(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ACHILLES_CH_TEEaccum_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ACHILLES_CH_TEEaccum_t* ms = SGX_CAST(ms_ACHILLES_CH_TEEaccum_t*, pms);
	ms_ACHILLES_CH_TEEaccum_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ACHILLES_CH_TEEaccum_t), ms, sizeof(ms_ACHILLES_CH_TEEaccum_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	onejusts_t* _tmp_js = __in_ms.ms_js;
	...
	_in_retval = ACHILLES_CH_TEEaccum(_in_js, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_ACHILLES_CH_TEEaccumSp(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ACHILLES_CH_TEEaccumSp_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ACHILLES_CH_TEEaccumSp_t* ms = SGX_CAST(ms_ACHILLES_CH_TEEaccumSp_t*, pms);
	ms_ACHILLES_CH_TEEaccumSp_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ACHILLES_CH_TEEaccumSp_t), ms, sizeof(ms_ACHILLES_CH_TEEaccumSp_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	just_t* _tmp_just = __in_ms.ms_just;
	...
	_in_retval = ACHILLES_CH_TEEaccumSp(_in_just, _in_res);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
SGX_EXTERNC const struct {
	...
	41,
	{
		...
		{(void*)(uintptr_t)sgx_DAMYSUS_TEEsign, 0, 0},
		{(void*)(uintptr_t)sgx_DAMYSUS_TEEprepare, 0, 0},
		{(void*)(uintptr_t)sgx_DAMYSUS_TEEstore, 0, 0},
		{(void*)(uintptr_t)sgx_DAMYSUS_TEEaccum, 0, 0},
		{(void*)(uintptr_t)sgx_DAMYSUS_TEEaccumSp, 0, 0},
		...
		{(void*)(uintptr_t)sgx_ACHILLES_CH_TEEsign, 0, 0},
		{(void*)(uintptr_t)sgx_ACHILLES_CH_TEEverify, 0, 0},
		{(void*)(uintptr_t)sgx_ACHILLES_CH_TEEprepare, 0, 0},
		{(void*)(uintptr_t)sgx_ACHILLES_CH_TEEaccum, 0, 0},
		{(void*)(uintptr_t)sgx_ACHILLES_CH_TEEaccumSp, 0, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[14][41];
} g_dyn_entry_table = {
	14,
	{
